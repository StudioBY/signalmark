        ...
        {samples.map((s) => (
          <Link key={s.slug} to={`/sample/${s.slug}`} className="group block text-left">
            {s.photo_url ? (
              ...
              <div className="h-14 w-14 rounded-full border border-neutral-200" />
            )}
            <p className="mt-4 text-[11px] uppercase leading-[1.5] tracking-[0.18em] text-[#1B2430] group-hover:text-neutral-500">
              {s.name}
            </p>

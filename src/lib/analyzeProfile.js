...
const SCHEMA = {
  ...
  properties: {
    ...
    rewrites: {
      ...
      items: {
        type: "object",
        properties: { original: { type: "string" }, revised: { type: "string" }, rationale: { type: "string" } },
      },
    },
  },
};

export async function analyzeProfile({ headline, about, posts }) {
  const prompt = `You are a computational linguistics engine that audits LinkedIn profile text. You do NOT coach and you do NOT use marketing language. You measure how the text is written, using observable, text-derived signals only. Tone: cold, precise, analytical, in English.
